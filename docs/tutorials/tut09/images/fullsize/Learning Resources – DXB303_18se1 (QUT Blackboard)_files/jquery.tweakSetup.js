...
Version 3.3.4 -- client viewable version of BB Manifest
Copyright 2014 Christopher Baldwin, Tim Plaisted, Queensland University of Technology
...
var tweak_live_help = "/webapps/blackboard/execute/courseMain?course_id=_59724_1";
var tweak_bb_editmode_css_pattern = "#editModeToggleLink.read-on";

// static code: tweak currently compatible with 8, 9 and 9.1
function BBVersionSetup() {
  // namespace setup and display view
  window.tweak_bb = {
    display_view: (jQuery(tweak_bb_editmode_css_pattern).length == 0)
  };
  // version detect
  ...
  var BB8, BB9, BB9_x;
  BB8 = BB9 = BB9_x = false;
  BB9_x = (jQuery(bb_page_id_9_x)
    .length == 1);
  if (!BB9_x) {
    BB9 = (jQuery(bb_page_id_9)
      .length == 1);
    if (!BB9) { // make BB8 look like 9
      BB8 = true;
      var page = tweak_bb.display_view ? jQuery("h1.pageTitle")
        .next("table") : jQuery("#endActionBar ~ table:eq(0)");
...
jQuery(function ($) {
  if (typeof (window.tweak_bb) === 'undefined') {
    BBVersionSetup();
  }
  if (tweak_bb.display_view) {
    $(tweak_bb.page_id + " script.tweak_script")
      .parents(tweak_bb.row_element)
      .hide();
  }
});
    ...
    this.load = function (options) {
      ...
      function load(type, url) {
        self[type](url, function () {
            ...
            d.js.length == loaded.js.length && d.css.length == loaded.css.length && d.image.length == loaded.image.length && d.load.apply(loaded, []);
            return;
          }, d.name ? 'lazy-loaded-' + d.name : 'lazy-loaded-' + new Date()
          .getTime());
      }
