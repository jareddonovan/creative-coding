...
function preload(){
  table = loadTable("data/guests.csv", "csv", "header");
}
...
function setup() {
  ...
  // // Create a new table object with columns: id, name, date
  // table = new p5.Table();
  // table.addColumn('id');
...
function nameSubmitted(){
  ...
  // Create a new row for the table and populate it.
  let newRow = table.addRow();
  newRow.setNum('id', table.getRowCount() - 1);
  newRow.setString('name', name);
  newRow.setString('date', d.toISOString());
...
function keyPressed(){
  if (key == 's'){
    saveTable(table, "downloaded-guests.csv");
  }
}
